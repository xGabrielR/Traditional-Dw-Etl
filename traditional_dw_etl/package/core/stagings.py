...
from package.utils.deco_functions import retry_get_staging_data, mem_profile
...
class SetupStages(object):
    """
    Initial class for setup and checkout all stagings from mysql and postgresql.
    """
    def __init__(
            self, 
            stagings_path: str | list,
            credentials_dict: dict,
            lookback_process_days: int = None,
            config_backend_dict: dict = None
        ):
        self.stagings_query = None
        self.stagings_names = None
        self.stagings_path = stagings_path
        self._process_date = lookback_process_days

        if type(stagings_path) not in [str, list]:
            raise TypeError('Stagings Path need to be a String path or List of paths.')

        if isinstance(stagings_path, list):
            self.backend_query_setup = []

            for query_path in stagings_path:
        ...
        if isinstance(stagings_path, str):
            stg_name, stg_query = self.checkout_stages_inputs(stagings_path)
            self.backend_query_setup = [{stagings_path.split('_')[-1]: [stg_name, stg_query]}]
    ...
    def checkout_stages_inputs(self, stagings_path):
        all_stages_paths = glob(stagings_path + '/*.sql')
...
class RunStagings(object):
    ...
    def __init__(
            ...
            logs_path: str = None,
            lookback_process_days: int = None,
            stagings_backend_config: dict = None
        ):
        self._logs_path = logs_path
        self._credentials_dict = credentials_dict
        self._schema=schema
        self.stagings_config = SetupStages(
            stagings_path=stagings_path,
            credentials_dict=credentials_dict,
            lookback_process_days=lookback_process_days
        )

    def get_backends(self, stagings_config):
        backends = ["".join(k.keys()) for k in list(stagings_config)]
    ...
    async def get_connections(self, only_ssms=False):
        if only_ssms == True:
            ...
            connections = {}
            for backend, uri in self.get_backends(self.stagings_config).items():
    ...
    async def staging_raw_data(self, connections):
        raw_stg_data = []
        for itters in list(self.stagings_config):
            ...
            for stg_name, stg_query in zip(stg_names, stg_queries):
                print(f'[{datetime.now().strftime("%Y-%m-%d %H:%M:%S")}] | [INFO] | START STAGING: {stg_name.upper()}', file=open(self._logs_path, 'a'))

                raw_data = await self.get_staging_data(connection, stg_query, stg_name)
                raw_stg_data.append(raw_data)

        return raw_stg_data
    

    @retry_get_staging_data()
    async def get_staging_data(self, connection, stg_query, stg_name) -> dict:
    ...
    async def data_storange(self, engine, stage: dict, schema: str) -> None:
        ...
        # Skip Retry Empty Staging Collect
        if not df.empty:
            df.to_sql(
                table_name.upper(),
                con=engine,
    ...
    @mem_profile
    async def run_stagings(self):
        connections = await self.get_connections()

        raw_stg_data = await self.staging_raw_data(connections)
